                    ...
                    <div className={`card ${cardBgClass} shadow-sm  h-100`}>
                        <div className="card-body d-flex flex-column">
                            ...
                            <p className="card-text mb-4">
                                {(() => {
                                    const ops0 = cardData?.content?.ops?.[0];
                                    const insert = ops0?.insert;

                                    if (typeof insert === 'string') {
                                        const preview = insert.slice(0, 50);
                                        return insert.length > 50 ? `${preview}...` : preview;
                                    }

                                    // If insert isn't a string (e.g., embed objects), don't crash the UI.
                                    if (insert == null) return '';

                                    const asString = typeof insert === 'number' ? String(insert) : JSON.stringify(insert);
                                    return asString.length > 50 ? `${asString.slice(0, 50)}...` : asString;
                                })()}
                            </p>
                            {/* Action Buttons */}
